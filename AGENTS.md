...
- The presence trigger ignores shadows (`AGENT_SHADOW_FILTER`, on by default): a brightness change counts only if `trigger.texture_difference` — the texture of log(frame / background) — also passes `AGENT_SHADOW_TEXTURE_THRESHOLD`. Without it, tree and vehicle shadows produced most `no_plate` events at the west gate. `AGENT_MOVING_READ_SECONDS` is 1.0 so a vehicle driving through is read while its body is still in the zone, not later on its shadow.
