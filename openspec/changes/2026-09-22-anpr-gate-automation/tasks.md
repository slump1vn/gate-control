...
- [x] 6.6 Several cameras per gate, each watching one direction (`GateCamera`), with a per-gate exit policy; the agent runs one worker per camera and every decision records its camera and direction. Verified end to end with two cameras.
