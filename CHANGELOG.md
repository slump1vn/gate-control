...
- `/monitor` page (refresh at most once a second, since the camera also serves the gate agent): live view of each gate lane (snapshots proxied through `/api/v1/gate/cameras/<id>/snapshot/`), the read zone drawn over the frame, camera and controller status, barrier arm, and a rolling list of recent decisions
